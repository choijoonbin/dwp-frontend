...
const backendSourceIsExplicit = Boolean(configuredSource);
...
if (mode === '--check') {
  if (fs.existsSync(syncSource)) {
    if (readCanonicalJson(syncSource) !== readCanonicalJson(snapshot)) {
      console.error(
    ...
    console.error(`Configured Backend Gateway contract is missing: ${syncSource}`);
    process.exit(1);
  }
}
