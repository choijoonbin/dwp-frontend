...
DWP는 네이티브 메일 도메인 `APP.MAIL`과 일정 도메인 `APP.CALENDAR`를 독립된 권한 및
데이터 경계로 운영한다. 일정 도메인은 일정·참석·반복·자원 예약·조직 정책·감사를
소유하며, 메일에서 발견한 일정 후보는 승인 전 제안으로만 Calendar에 전달한다. 외부
공급사 일정은 Adapter가 Calendar 모델에 투영한다.
