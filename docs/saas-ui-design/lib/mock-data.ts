// Types
export interface Tenant {
  id: bigint
  name: string
  code: string
}

export interface CompanyCode {
  id: string
  name: string
  tenantId: bigint
}

export interface Case {
  id: string
  caseNumber: string
  title: string
  severity: 'critical' | 'high' | 'medium' | 'low'
  status: 'open' | 'in_progress' | 'pending_approval' | 'resolved' | 'dismissed'
  anomalyType: 'duplicate_invoice' | 'bank_change' | 'policy_violation' | 'integrity_mismatch' | 'amount_variance' | 'timing_anomaly'
  tenantId: bigint
  companyCode: string
  counterparty: string
  counterpartyId: string
  amount: number
  currency: string
  detectedAt: string
  slaDue: string
  assignee: string | null
  confidence: number
  fiDocId: string
  description: string
}

export interface Action {
  id: string
  caseId: string
  actionType: 'post_reversal' | 'block_payment' | 'flag_review' | 'clear_item' | 'update_master'
  autonomyMode: 'auto' | 'semi_auto' | 'manual'
  requiredApproval: boolean
  riskLevel: 'critical' | 'high' | 'medium' | 'low'
  targetSystem: string
  createdAt: string
  status: 'pending' | 'approved' | 'rejected' | 'executed' | 'failed'
  description: string
  simulationResult?: SimulationResult
}

export interface SimulationResult {
  predictedSuccess: boolean
  impactedObjects: string[]
  validations: { name: string; passed: boolean; message: string }[]
  riskNotes: string[]
}

export interface FiDocHeader {
  id: string
  belnr: string // Document Number
  bukrs: string // Company Code
  gjahr: string // Fiscal Year
  budat: string // Posting Date
  bldat: string // Document Date
  blart: string // Document Type
  tcode: string // Transaction Code
  usnam: string // Created By User
  counterparty: string // Vendor/Customer Name
  counterpartyId: string // Vendor/Customer ID
  wrbtr: number // Amount in Document Currency
  waers: string // Currency Key
  xblnr: string // Reference
  bktxt: string // Document Header Text
  tenantId: bigint
  integrityStatus: 'pass' | 'warn' | 'fail'
  reversalFlag: boolean
  reversedByDoc?: string
  reversesDoc?: string
  createdAt: string
  linkedCasesCount: number
}

export interface FiDocItem {
  id: string
  docId: string
  buzei: number // Line Item Number
  hkont: string // G/L Account
  hkontName: string // G/L Account Name
  shkzg: 'S' | 'H' // Debit/Credit Indicator (S=Debit, H=Credit)
  wrbtr: number // Amount
  mwskz: string // Tax Code
  kostl: string // Cost Center
  prctr: string // Profit Center
  zuonr: string // Assignment
  sgtxt: string // Item Text
}

export interface IntegrityCheck {
  id: string
  docId: string
  ruleName: string
  severity: 'info' | 'warn' | 'critical'
  passed: boolean
  evidence: string
  recommendation: string
  relatedCaseId?: string
}

export interface OpenItem {
  id: string
  entityId: string
  entityName: string
  docId: string
  docNumber: string
  type: 'AR' | 'AP'
  amount: number
  currency: string
  dueDate: string
  daysPastDue: number
  status: 'open' | 'partially_cleared' | 'cleared'
  disputeFlag: boolean
  paymentBlock: boolean
  blockReason?: string
  recommendedAction?: 'send_reminder' | 'request_approval' | 'create_case' | 'escalate' | 'auto_clear'
  guardrailStatus?: 'allowed' | 'approval_required' | 'blocked'
  companyCode: string
  tenantId: bigint
  clearingHistory?: { date: string; amount: number; clearingDoc: string }[]
}

export interface Entity {
  id: string
  type: 'vendor' | 'customer'
  code: string
  name: string
  tenantId: bigint
  companyCode: string
  riskScore: number
  riskTrend: 'up' | 'down' | 'stable'
  concentrationRisk: 'low' | 'medium' | 'high'
  lastUpdated: string
  openItemsTotal: number
  openItemsCount: number
  overdueTotal: number
  overdueCount: number
  recentAnomaliesCount: number
  currency: string
  // PII fields (may be masked)
  bankAccount?: string
  bankName?: string
  contactName?: string
  contactEmail?: string
  contactPhone?: string
  address?: string
  taxId?: string
  paymentTerms?: string
  // Linked data
  linkedDocIds: string[]
  linkedCaseIds: string[]
  linkedOpenItemIds: string[]
}

export interface EntityChangeLog {
  id: string
  entityId: string
  timestamp: string
  fieldName: string
  beforeValue: string
  afterValue: string
  actor: string
  actorType: 'system' | 'user' | 'agent'
  source: string
  severity: 'info' | 'warn' | 'critical'
}

export interface AuditEvent {
  id: string
  caseId?: string
  timestamp: string
  eventType: 'case_created' | 'action_proposed' | 'simulation_run' | 'approval_requested' | 'action_approved' | 'action_rejected' | 'action_executed' | 'comment_added'
  actor: string
  actorType: 'system' | 'user' | 'agent'
  description: string
  metadata?: Record<string, unknown>
}

export interface Policy {
  id: string
  name: string
  category: string
  content: string
  source: string
  lastUpdated: string
}

export interface Comment {
  id: string
  caseId: string
  author: string
  authorType?: 'user' | 'ai' | 'system'
  content: string
  createdAt: string
  attachments?: string[]
}

// Mock Tenants
export const mockTenants: Tenant[] = [
  { id: BigInt(1001), name: 'Acme Corporation', code: 'ACME' },
  { id: BigInt(1002), name: 'Global Industries', code: 'GLBL' },
  { id: BigInt(1003), name: 'Tech Solutions Inc', code: 'TECH' },
]

// Mock Company Codes
export const mockCompanyCodes: CompanyCode[] = [
  { id: '1000', name: 'ACME US', tenantId: BigInt(1001) },
  { id: '2000', name: 'ACME EU', tenantId: BigInt(1001) },
  { id: '3000', name: 'ACME APAC', tenantId: BigInt(1001) },
  { id: '4000', name: 'Global US', tenantId: BigInt(1002) },
  { id: '5000', name: 'Global EU', tenantId: BigInt(1002) },
]

// Mock Cases
export const mockCases: Case[] = [
  {
    id: 'case-001',
    caseNumber: 'CS-2026-0001',
    title: 'Duplicate Invoice - Vendor Alpha Inc',
    severity: 'critical',
    status: 'pending_approval',
    anomalyType: 'duplicate_invoice',
    tenantId: BigInt(1001),
    companyCode: '1000',
    counterparty: 'Vendor Alpha Inc',
    counterpartyId: 'V-10001',
    amount: 125000.00,
    currency: 'USD',
    detectedAt: '2026-01-28T14:32:00Z',
    slaDue: '2026-01-30T14:32:00Z',
    assignee: 'John Smith',
    confidence: 94,
    fiDocId: 'fi-001',
    description: 'Potential duplicate invoice detected with 94% confidence. Invoice amount $125,000 matches previous payment within 30 days.'
  },
  {
    id: 'case-002',
    caseNumber: 'CS-2026-0002',
    title: 'Bank Account Change - Supplier Beta LLC',
    severity: 'high',
    status: 'open',
    anomalyType: 'bank_change',
    tenantId: BigInt(1001),
    companyCode: '1000',
    counterparty: 'Supplier Beta LLC',
    counterpartyId: 'V-10002',
    amount: 89500.00,
    currency: 'USD',
    detectedAt: '2026-01-29T09:15:00Z',
    slaDue: '2026-02-01T09:15:00Z',
    assignee: null,
    confidence: 87,
    fiDocId: 'fi-002',
    description: 'Bank account change detected 24 hours before scheduled payment. High-risk pattern identified.'
  },
  {
    id: 'case-003',
    caseNumber: 'CS-2026-0003',
    title: 'Authorization Limit Exceeded - Contractor Gamma',
    severity: 'high',
    status: 'in_progress',
    anomalyType: 'policy_violation',
    tenantId: BigInt(1001),
    companyCode: '2000',
    counterparty: 'Contractor Gamma',
    counterpartyId: 'V-10003',
    amount: 52000.00,
    currency: 'EUR',
    detectedAt: '2026-01-29T11:45:00Z',
    slaDue: '2026-02-02T11:45:00Z',
    assignee: 'Sarah Johnson',
    confidence: 91,
    fiDocId: 'fi-003',
    description: 'Payment exceeds single transaction limit without required dual approval.'
  },
  {
    id: 'case-004',
    caseNumber: 'CS-2026-0004',
    title: 'PO/Invoice Mismatch - Vendor Delta Corp',
    severity: 'medium',
    status: 'open',
    anomalyType: 'integrity_mismatch',
    tenantId: BigInt(1001),
    companyCode: '1000',
    counterparty: 'Vendor Delta Corp',
    counterpartyId: 'V-10004',
    amount: 34200.00,
    currency: 'USD',
    detectedAt: '2026-01-30T08:20:00Z',
    slaDue: '2026-02-03T08:20:00Z',
    assignee: null,
    confidence: 78,
    fiDocId: 'fi-004',
    description: 'Invoice line items do not match PO quantities. Variance of 15% detected.'
  },
  {
    id: 'case-005',
    caseNumber: 'CS-2026-0005',
    title: 'Off-hours Payment - Supplier Epsilon',
    severity: 'low',
    status: 'resolved',
    anomalyType: 'timing_anomaly',
    tenantId: BigInt(1001),
    companyCode: '3000',
    counterparty: 'Supplier Epsilon',
    counterpartyId: 'V-10005',
    amount: 15800.00,
    currency: 'USD',
    detectedAt: '2026-01-27T16:00:00Z',
    slaDue: '2026-01-31T16:00:00Z',
    assignee: 'Mike Chen',
    confidence: 65,
    fiDocId: 'fi-005',
    description: 'Payment scheduled outside normal business hours. Verified as legitimate after review.'
  },
  {
    id: 'case-006',
    caseNumber: 'CS-2026-0006',
    title: 'Unusual Amount - Global Vendor A',
    severity: 'critical',
    status: 'open',
    anomalyType: 'amount_variance',
    tenantId: BigInt(1002),
    companyCode: '4000',
    counterparty: 'Global Vendor A',
    counterpartyId: 'V-20001',
    amount: 275000.00,
    currency: 'USD',
    detectedAt: '2026-01-30T10:30:00Z',
    slaDue: '2026-01-31T10:30:00Z',
    assignee: null,
    confidence: 96,
    fiDocId: 'fi-006',
    description: 'Invoice amount 3x higher than historical average for this vendor.'
  },
]

// Mock Actions
export const mockActions: Action[] = [
  {
    id: 'act-001',
    caseId: 'case-001',
    actionType: 'block_payment',
    autonomyMode: 'semi_auto',
    requiredApproval: true,
    riskLevel: 'critical',
    targetSystem: 'SAP FI',
    createdAt: '2026-01-28T14:35:00Z',
    status: 'pending',
    description: 'Block payment run for invoice #INV-2026-1234 pending duplicate verification',
    simulationResult: {
      predictedSuccess: true,
      impactedObjects: ['Payment Run PR-2026-001', 'Invoice INV-2026-1234', 'Vendor Balance V-10001'],
      validations: [
        { name: 'Authorization Check', passed: true, message: 'User has required permissions' },
        { name: 'Business Rule Check', passed: true, message: 'Within policy guidelines' },
        { name: 'System Availability', passed: true, message: 'SAP FI system available' },
      ],
      riskNotes: ['Payment delay may affect vendor relationship', 'Manual follow-up required within 24h']
    }
  },
  {
    id: 'act-002',
    caseId: 'case-002',
    actionType: 'flag_review',
    autonomyMode: 'auto',
    requiredApproval: false,
    riskLevel: 'high',
    targetSystem: 'SAP MM',
    createdAt: '2026-01-29T09:20:00Z',
    status: 'executed',
    description: 'Flag vendor master data for compliance review due to bank change'
  },
  {
    id: 'act-003',
    caseId: 'case-003',
    actionType: 'post_reversal',
    autonomyMode: 'manual',
    requiredApproval: true,
    riskLevel: 'high',
    targetSystem: 'SAP FI',
    createdAt: '2026-01-29T12:00:00Z',
    status: 'approved',
    description: 'Post reversal document for unauthorized payment'
  },
  {
    id: 'act-004',
    caseId: 'case-001',
    actionType: 'clear_item',
    autonomyMode: 'semi_auto',
    requiredApproval: true,
    riskLevel: 'medium',
    targetSystem: 'SAP FI',
    createdAt: '2026-01-30T08:00:00Z',
    status: 'pending',
    description: 'Clear open item after duplicate confirmation'
  },
]

// Mock FI Documents
export const mockFiDocs: FiDocHeader[] = [
  {
    id: 'fi-001',
    belnr: '1900001234',
    bukrs: '1000',
    gjahr: '2026',
    budat: '2026-01-28',
    bldat: '2026-01-25',
    blart: 'KR',
    tcode: 'FB60',
    usnam: 'BATCH_USER',
    counterparty: 'Vendor Alpha Inc',
    counterpartyId: 'V-10001',
    wrbtr: 125000.00,
    waers: 'USD',
    xblnr: 'INV-2026-1234',
    bktxt: 'Vendor Invoice - Materials Q1',
    tenantId: BigInt(1001),
    integrityStatus: 'warn',
    reversalFlag: false,
    createdAt: '2026-01-28T14:30:00Z',
    linkedCasesCount: 1
  },
  {
    id: 'fi-002',
    belnr: '1900001235',
    bukrs: '1000',
    gjahr: '2026',
    budat: '2026-01-29',
    bldat: '2026-01-27',
    blart: 'KR',
    tcode: 'FB60',
    usnam: 'AP_USER01',
    counterparty: 'Supplier Beta LLC',
    counterpartyId: 'V-10002',
    wrbtr: 89500.00,
    waers: 'USD',
    xblnr: 'INV-2026-5678',
    bktxt: 'Services Invoice - Consulting',
    tenantId: BigInt(1001),
    integrityStatus: 'pass',
    reversalFlag: false,
    createdAt: '2026-01-29T09:10:00Z',
    linkedCasesCount: 1
  },
  {
    id: 'fi-003',
    belnr: '1900001236',
    bukrs: '2000',
    gjahr: '2026',
    budat: '2026-01-29',
    bldat: '2026-01-28',
    blart: 'KR',
    tcode: 'MIRO',
    usnam: 'AP_USER02',
    counterparty: 'Contractor Gamma',
    counterpartyId: 'V-10003',
    wrbtr: 52000.00,
    waers: 'EUR',
    xblnr: 'INV-EU-2026-001',
    bktxt: 'Project Services - Phase 2',
    tenantId: BigInt(1001),
    integrityStatus: 'fail',
    reversalFlag: false,
    createdAt: '2026-01-29T11:45:00Z',
    linkedCasesCount: 1
  },
  {
    id: 'fi-004',
    belnr: '1900001237',
    bukrs: '1000',
    gjahr: '2026',
    budat: '2026-01-30',
    bldat: '2026-01-29',
    blart: 'KR',
    tcode: 'FB60',
    usnam: 'AP_USER01',
    counterparty: 'Vendor Delta Corp',
    counterpartyId: 'V-10004',
    wrbtr: 34200.00,
    waers: 'USD',
    xblnr: 'INV-2026-9012',
    bktxt: 'Office Supplies Q1',
    tenantId: BigInt(1001),
    integrityStatus: 'warn',
    reversalFlag: false,
    createdAt: '2026-01-30T08:20:00Z',
    linkedCasesCount: 1
  },
  {
    id: 'fi-005',
    belnr: '1900001238',
    bukrs: '3000',
    gjahr: '2026',
    budat: '2026-01-27',
    bldat: '2026-01-26',
    blart: 'KZ',
    tcode: 'F110',
    usnam: 'PAYMENT_BOT',
    counterparty: 'Supplier Epsilon',
    counterpartyId: 'V-10005',
    wrbtr: 15800.00,
    waers: 'USD',
    xblnr: 'PAY-2026-001',
    bktxt: 'Payment Run - Batch 001',
    tenantId: BigInt(1001),
    integrityStatus: 'pass',
    reversalFlag: false,
    createdAt: '2026-01-27T16:00:00Z',
    linkedCasesCount: 1
  },
  {
    id: 'fi-006',
    belnr: '1900001239',
    bukrs: '4000',
    gjahr: '2026',
    budat: '2026-01-30',
    bldat: '2026-01-29',
    blart: 'KR',
    tcode: 'FB60',
    usnam: 'AP_MANAGER',
    counterparty: 'Global Vendor A',
    counterpartyId: 'V-20001',
    wrbtr: 275000.00,
    waers: 'USD',
    xblnr: 'INV-GLB-2026-100',
    bktxt: 'Large Equipment Purchase',
    tenantId: BigInt(1002),
    integrityStatus: 'fail',
    reversalFlag: false,
    createdAt: '2026-01-30T10:30:00Z',
    linkedCasesCount: 1
  },
  // Documents with reversal chains
  {
    id: 'fi-007',
    belnr: '1900001100',
    bukrs: '1000',
    gjahr: '2026',
    budat: '2026-01-15',
    bldat: '2026-01-14',
    blart: 'KR',
    tcode: 'FB60',
    usnam: 'AP_USER01',
    counterparty: 'Vendor Alpha Inc',
    counterpartyId: 'V-10001',
    wrbtr: 45000.00,
    waers: 'USD',
    xblnr: 'INV-2026-0050',
    bktxt: 'Original Invoice - Reversed',
    tenantId: BigInt(1001),
    integrityStatus: 'pass',
    reversalFlag: true,
    reversedByDoc: 'fi-008',
    createdAt: '2026-01-15T10:00:00Z',
    linkedCasesCount: 0
  },
  {
    id: 'fi-008',
    belnr: '1900001101',
    bukrs: '1000',
    gjahr: '2026',
    budat: '2026-01-20',
    bldat: '2026-01-20',
    blart: 'KR',
    tcode: 'FB08',
    usnam: 'AP_MANAGER',
    counterparty: 'Vendor Alpha Inc',
    counterpartyId: 'V-10001',
    wrbtr: -45000.00,
    waers: 'USD',
    xblnr: 'REV-2026-0050',
    bktxt: 'Reversal of 1900001100',
    tenantId: BigInt(1001),
    integrityStatus: 'pass',
    reversalFlag: true,
    reversesDoc: 'fi-007',
    reversedByDoc: 'fi-009',
    createdAt: '2026-01-20T14:00:00Z',
    linkedCasesCount: 0
  },
  {
    id: 'fi-009',
    belnr: '1900001102',
    bukrs: '1000',
    gjahr: '2026',
    budat: '2026-01-22',
    bldat: '2026-01-22',
    blart: 'KR',
    tcode: 'FB08',
    usnam: 'AP_MANAGER',
    counterparty: 'Vendor Alpha Inc',
    counterpartyId: 'V-10001',
    wrbtr: 45000.00,
    waers: 'USD',
    xblnr: 'REV-2026-0051',
    bktxt: 'Re-reversal of 1900001101',
    tenantId: BigInt(1001),
    integrityStatus: 'pass',
    reversalFlag: false,
    reversesDoc: 'fi-008',
    createdAt: '2026-01-22T09:00:00Z',
    linkedCasesCount: 0
  },
  // More documents for variety
  {
    id: 'fi-010',
    belnr: '1900001240',
    bukrs: '1000',
    gjahr: '2026',
    budat: '2026-01-25',
    bldat: '2026-01-24',
    blart: 'SA',
    tcode: 'FB50',
    usnam: 'GL_USER01',
    counterparty: 'Internal Transfer',
    counterpartyId: '',
    wrbtr: 500000.00,
    waers: 'USD',
    xblnr: 'JE-2026-001',
    bktxt: 'Month-end Accrual Entry',
    tenantId: BigInt(1001),
    integrityStatus: 'pass',
    reversalFlag: false,
    createdAt: '2026-01-25T23:45:00Z',
    linkedCasesCount: 0
  },
  {
    id: 'fi-011',
    belnr: '1900001241',
    bukrs: '1000',
    gjahr: '2026',
    budat: '2026-01-26',
    bldat: '2026-01-25',
    blart: 'DR',
    tcode: 'FB70',
    usnam: 'AR_USER01',
    counterparty: 'Customer XYZ Corp',
    counterpartyId: 'C-50001',
    wrbtr: 78500.00,
    waers: 'USD',
    xblnr: 'INV-OUT-2026-100',
    bktxt: 'Customer Invoice - Project A',
    tenantId: BigInt(1001),
    integrityStatus: 'pass',
    reversalFlag: false,
    createdAt: '2026-01-26T11:30:00Z',
    linkedCasesCount: 0
  },
  {
    id: 'fi-012',
    belnr: '1900001242',
    bukrs: '2000',
    gjahr: '2026',
    budat: '2026-01-28',
    bldat: '2026-01-27',
    blart: 'KR',
    tcode: 'FB60',
    usnam: 'AP_USER03',
    counterparty: 'EU Supplier GmbH',
    counterpartyId: 'V-30001',
    wrbtr: 125000.00,
    waers: 'EUR',
    xblnr: 'INV-EU-2026-050',
    bktxt: 'IT Equipment Purchase',
    tenantId: BigInt(1001),
    integrityStatus: 'warn',
    reversalFlag: false,
    createdAt: '2026-01-28T08:15:00Z',
    linkedCasesCount: 0
  },
]

// Mock FI Document Items
export const mockFiDocItems: FiDocItem[] = [
  // Items for fi-001
  {
    id: 'item-001',
    docId: 'fi-001',
    buzei: 1,
    hkont: '21000',
    hkontName: 'Accounts Payable',
    shkzg: 'H',
    wrbtr: 125000.00,
    mwskz: 'V0',
    kostl: '',
    prctr: '',
    zuonr: 'V-10001',
    sgtxt: 'Vendor Invoice INV-2026-1234'
  },
  {
    id: 'item-002',
    docId: 'fi-001',
    buzei: 2,
    hkont: '60100',
    hkontName: 'Cost of Goods Sold',
    shkzg: 'S',
    wrbtr: 112500.00,
    mwskz: 'V0',
    kostl: 'CC1000',
    prctr: 'PC1000',
    zuonr: 'PO-2026-001',
    sgtxt: 'Material costs'
  },
  {
    id: 'item-003',
    docId: 'fi-001',
    buzei: 3,
    hkont: '15400',
    hkontName: 'Input Tax',
    shkzg: 'S',
    wrbtr: 12500.00,
    mwskz: 'V1',
    kostl: '',
    prctr: '',
    zuonr: '',
    sgtxt: 'Input tax 10%'
  },
  // Items for fi-002
  {
    id: 'item-004',
    docId: 'fi-002',
    buzei: 1,
    hkont: '21000',
    hkontName: 'Accounts Payable',
    shkzg: 'H',
    wrbtr: 89500.00,
    mwskz: 'V0',
    kostl: '',
    prctr: '',
    zuonr: 'V-10002',
    sgtxt: 'Vendor Invoice INV-2026-5678'
  },
  {
    id: 'item-005',
    docId: 'fi-002',
    buzei: 2,
    hkont: '62000',
    hkontName: 'Professional Services',
    shkzg: 'S',
    wrbtr: 80550.00,
    mwskz: 'V0',
    kostl: 'CC2000',
    prctr: 'PC2000',
    zuonr: 'WBS-2026-001',
    sgtxt: 'Consulting services'
  },
  {
    id: 'item-006',
    docId: 'fi-002',
    buzei: 3,
    hkont: '15400',
    hkontName: 'Input Tax',
    shkzg: 'S',
    wrbtr: 8950.00,
    mwskz: 'V1',
    kostl: '',
    prctr: '',
    zuonr: '',
    sgtxt: 'Input tax 10%'
  },
  // Items for fi-003
  {
    id: 'item-007',
    docId: 'fi-003',
    buzei: 1,
    hkont: '21000',
    hkontName: 'Accounts Payable',
    shkzg: 'H',
    wrbtr: 52000.00,
    mwskz: 'V0',
    kostl: '',
    prctr: '',
    zuonr: 'V-10003',
    sgtxt: 'Vendor Invoice INV-EU-2026-001'
  },
  {
    id: 'item-008',
    docId: 'fi-003',
    buzei: 2,
    hkont: '62000',
    hkontName: 'Project Expenses',
    shkzg: 'S',
    wrbtr: 52000.00,
    mwskz: '',
    kostl: 'CC3000',
    prctr: 'PC3000',
    zuonr: 'PROJ-2026-002',
    sgtxt: 'Phase 2 deliverables'
  },
]

// Mock Integrity Checks
export const mockIntegrityChecks: IntegrityCheck[] = [
  {
    id: 'chk-001',
    docId: 'fi-001',
    ruleName: 'Duplicate Invoice Detection',
    severity: 'warn',
    passed: false,
    evidence: 'Found matching invoice INV-2025-9999 with same amount ($125,000) from same vendor within 30 days',
    recommendation: 'Review potential duplicate and confirm legitimacy',
    relatedCaseId: 'case-001'
  },
  {
    id: 'chk-002',
    docId: 'fi-001',
    ruleName: 'Header-Line Total Match',
    severity: 'info',
    passed: true,
    evidence: 'Header total $125,000 matches sum of line items',
    recommendation: ''
  },
  {
    id: 'chk-003',
    docId: 'fi-003',
    ruleName: 'Authorization Limit Check',
    severity: 'critical',
    passed: false,
    evidence: 'Document amount EUR 52,000 exceeds single approval limit of EUR 50,000',
    recommendation: 'Requires dual authorization per Policy PAM-001',
    relatedCaseId: 'case-003'
  },
  {
    id: 'chk-004',
    docId: 'fi-003',
    ruleName: 'Tax Code Validation',
    severity: 'warn',
    passed: false,
    evidence: 'Line item 2 missing tax code for EU cross-border transaction',
    recommendation: 'Verify tax treatment with Tax department'
  },
  {
    id: 'chk-005',
    docId: 'fi-004',
    ruleName: 'PO/Invoice Matching',
    severity: 'warn',
    passed: false,
    evidence: 'Invoice quantity (100 units) differs from PO quantity (85 units) by 15%',
    recommendation: 'Confirm quantity variance with Procurement',
    relatedCaseId: 'case-004'
  },
  {
    id: 'chk-006',
    docId: 'fi-006',
    ruleName: 'Historical Amount Variance',
    severity: 'critical',
    passed: false,
    evidence: 'Invoice amount $275,000 is 3.2x higher than 12-month average ($85,937) for this vendor',
    recommendation: 'Confirm business justification for unusual amount',
    relatedCaseId: 'case-006'
  },
  {
    id: 'chk-007',
    docId: 'fi-010',
    ruleName: 'Posting Time Check',
    severity: 'info',
    passed: true,
    evidence: 'Document posted at 23:45 (outside normal hours) - flagged for month-end processing',
    recommendation: ''
  },
]

// Mock Audit Events
export const mockAuditEvents: AuditEvent[] = [
  {
    id: 'evt-001',
    caseId: 'case-001',
    timestamp: '2026-01-28T14:32:00Z',
    eventType: 'case_created',
    actor: 'AI Agent',
    actorType: 'agent',
    description: 'Case CS-2026-0001 created: Potential duplicate invoice detected'
  },
  {
    id: 'evt-002',
    caseId: 'case-001',
    timestamp: '2026-01-28T14:35:00Z',
    eventType: 'action_proposed',
    actor: 'AI Agent',
    actorType: 'agent',
    description: 'Action proposed: Block payment for INV-2026-1234'
  },
  {
    id: 'evt-003',
    caseId: 'case-001',
    timestamp: '2026-01-28T14:40:00Z',
    eventType: 'simulation_run',
    actor: 'AI Agent',
    actorType: 'agent',
    description: 'Pre-execution simulation completed successfully'
  },
  {
    id: 'evt-004',
    caseId: 'case-001',
    timestamp: '2026-01-28T15:00:00Z',
    eventType: 'approval_requested',
    actor: 'AI Agent',
    actorType: 'agent',
    description: 'Approval requested from John Smith for payment block'
  },
  {
    id: 'evt-005',
    caseId: 'case-001',
    timestamp: '2026-01-28T15:30:00Z',
    eventType: 'comment_added',
    actor: 'John Smith',
    actorType: 'user',
    description: 'Comment added: "Reviewing vendor history before approval"'
  },
]

// Mock Policies
export const mockPolicies: Policy[] = [
  {
    id: 'pol-001',
    name: 'Duplicate Invoice Detection Policy',
    category: 'AP Controls',
    content: 'Invoices with matching amounts within a 30-day window from the same vendor must be flagged for review. Threshold: >$10,000 or >90% amount match.',
    source: 'Corporate Finance Policy Manual v4.2',
    lastUpdated: '2025-12-01'
  },
  {
    id: 'pol-002',
    name: 'Bank Account Change Policy',
    category: 'Vendor Management',
    content: 'Any vendor bank account changes within 72 hours of a scheduled payment require dual approval and vendor callback verification.',
    source: 'Treasury Operations Guidelines',
    lastUpdated: '2025-11-15'
  },
  {
    id: 'pol-003',
    name: 'Payment Authorization Matrix',
    category: 'Authorization',
    content: 'Single transactions exceeding $50,000 require manager approval. Transactions exceeding $100,000 require director approval.',
    source: 'Delegation of Authority Policy',
    lastUpdated: '2025-10-20'
  },
]

// Mock Comments
export const mockComments: Comment[] = [
  {
    id: 'cmt-001',
    caseId: 'case-001',
    author: 'John Smith',
    authorType: 'user',
    content: 'Reviewing vendor history before approval. Checking previous invoices from Q4 2025.',
    createdAt: '2026-01-28T15:30:00Z'
  },
  {
    id: 'cmt-002',
    caseId: 'case-001',
    author: 'AI Agent',
    authorType: 'ai',
    content: 'Found 3 similar invoices in Q4 2025. Attached comparison report for reference.',
    createdAt: '2026-01-28T15:35:00Z',
    attachments: ['comparison-report.pdf']
  },
]

// Mock KPIs
export const mockKPIs = {
  financialHealthIndex: 87,
  financialHealthTrend: 2.3,
  openCasesBySeverity: {
    critical: 2,
    high: 2,
    medium: 1,
    low: 1
  },
  aiActionSuccessRate: 94.5,
  aiActionSuccessTrend: 1.2,
  estimatedPreventedLoss: 1250000,
  preventedLossTrend: 15.5,
  pendingApprovals: 3,
  slaAtRisk: 2,
  avgLeadTime: 4.2,
  backlogCount: 12
}

// Mock Agent Activity Stream
export const mockAgentActivity = [
  { id: 1, timestamp: '2026-01-30T10:45:32Z', action: 'SCAN', target: 'FI Documents', status: 'complete', message: 'Processed 1,247 documents in batch scan' },
  { id: 2, timestamp: '2026-01-30T10:45:28Z', action: 'DETECT', target: 'Case CS-2026-0006', status: 'alert', message: 'Critical anomaly detected: Amount variance 3x historical average' },
  { id: 3, timestamp: '2026-01-30T10:44:15Z', action: 'EXECUTE', target: 'Action ACT-002', status: 'success', message: 'Auto-flagged vendor V-10002 for compliance review' },
  { id: 4, timestamp: '2026-01-30T10:43:00Z', action: 'SIMULATE', target: 'Action ACT-001', status: 'complete', message: 'Pre-execution simulation passed all validations' },
  { id: 5, timestamp: '2026-01-30T10:42:30Z', action: 'ANALYZE', target: 'Vendor V-10001', status: 'complete', message: 'RAG analysis complete: 3 policy citations found' },
  { id: 6, timestamp: '2026-01-30T10:41:00Z', action: 'MATCH', target: 'Open Items', status: 'complete', message: 'Auto-matched 45 open items, 3 exceptions flagged' },
]

// Mock Risk Drivers
export const mockRiskDrivers = [
  { id: 1, type: 'duplicate_invoice', label: 'Duplicate Invoices', count: 12, amount: 450000, trend: 'up' },
  { id: 2, type: 'bank_change', label: 'Bank Change Risk', count: 5, amount: 280000, trend: 'stable' },
  { id: 3, type: 'policy_violation', label: 'Policy Violations', count: 8, amount: 320000, trend: 'down' },
  { id: 4, type: 'integrity_mismatch', label: 'Data Integrity', count: 15, amount: 180000, trend: 'up' },
]

// Mock Team Snapshot
export const mockTeamSnapshot = [
  { id: 1, name: 'John Smith', role: 'Senior Analyst', openCases: 5, slaRisk: 1, avgLeadTime: 3.2 },
  { id: 2, name: 'Sarah Johnson', role: 'AP Manager', openCases: 8, slaRisk: 2, avgLeadTime: 4.5 },
  { id: 3, name: 'Mike Chen', role: 'Analyst', openCases: 3, slaRisk: 0, avgLeadTime: 2.8 },
  { id: 4, name: 'Emily Davis', role: 'Analyst', openCases: 6, slaRisk: 1, avgLeadTime: 3.9 },
]

// Saved Views
export interface SavedView {
  id: string
  name: string
  filters: Record<string, unknown>
  columns: string[]
  isDefault?: boolean
}

export const mockSavedViews: SavedView[] = [
  { id: 'view-1', name: 'All Open Cases', filters: { status: ['open', 'in_progress'] }, columns: ['caseNumber', 'severity', 'status', 'anomalyType', 'counterparty', 'amount', 'slaDue', 'assignee'], isDefault: true },
  { id: 'view-2', name: 'Critical & High Priority', filters: { severity: ['critical', 'high'] }, columns: ['caseNumber', 'severity', 'anomalyType', 'counterparty', 'amount', 'slaDue', 'confidence'] },
  { id: 'view-3', name: 'My Assignments', filters: { assignee: 'current_user' }, columns: ['caseNumber', 'severity', 'status', 'counterparty', 'amount', 'slaDue'] },
  { id: 'view-4', name: 'Pending Approvals', filters: { status: ['pending_approval'] }, columns: ['caseNumber', 'severity', 'anomalyType', 'counterparty', 'amount', 'assignee'] },
]

// Mock Entities (Vendors & Customers)
export const mockEntities: Entity[] = [
  {
    id: 'V-10001',
    type: 'vendor',
    code: 'V-10001',
    name: 'Vendor Alpha Inc',
    tenantId: BigInt(1001),
    companyCode: '1000',
    riskScore: 78,
    riskTrend: 'up',
    concentrationRisk: 'high',
    lastUpdated: '2026-01-28T14:30:00Z',
    openItemsTotal: 170000,
    openItemsCount: 3,
    overdueTotal: 45000,
    overdueCount: 1,
    recentAnomaliesCount: 2,
    currency: 'USD',
    bankAccount: '****4567',
    bankName: 'Chase Bank',
    contactName: 'Robert Wilson',
    contactEmail: 'r.wilson@alpha-inc.com',
    contactPhone: '+1 555-0101',
    address: '123 Business Ave, New York, NY 10001',
    taxId: '12-3456789',
    paymentTerms: 'Net 30',
    linkedDocIds: ['fi-001', 'fi-007', 'fi-008', 'fi-009'],
    linkedCaseIds: ['case-001'],
    linkedOpenItemIds: ['oi-001', 'oi-002', 'oi-003']
  },
  {
    id: 'V-10002',
    type: 'vendor',
    code: 'V-10002',
    name: 'Supplier Beta LLC',
    tenantId: BigInt(1001),
    companyCode: '1000',
    riskScore: 65,
    riskTrend: 'stable',
    concentrationRisk: 'medium',
    lastUpdated: '2026-01-29T09:10:00Z',
    openItemsTotal: 89500,
    openItemsCount: 1,
    overdueTotal: 0,
    overdueCount: 0,
    recentAnomaliesCount: 1,
    currency: 'USD',
    bankAccount: '****8901',
    bankName: 'Bank of America',
    contactName: 'Lisa Chen',
    contactEmail: 'l.chen@betallc.com',
    contactPhone: '+1 555-0102',
    address: '456 Commerce St, Los Angeles, CA 90001',
    taxId: '98-7654321',
    paymentTerms: 'Net 45',
    linkedDocIds: ['fi-002'],
    linkedCaseIds: ['case-002'],
    linkedOpenItemIds: ['oi-004']
  },
  {
    id: 'V-10003',
    type: 'vendor',
    code: 'V-10003',
    name: 'Contractor Gamma',
    tenantId: BigInt(1001),
    companyCode: '2000',
    riskScore: 82,
    riskTrend: 'up',
    concentrationRisk: 'low',
    lastUpdated: '2026-01-29T11:45:00Z',
    openItemsTotal: 52000,
    openItemsCount: 1,
    overdueTotal: 52000,
    overdueCount: 1,
    recentAnomaliesCount: 1,
    currency: 'EUR',
    bankAccount: '****2345',
    bankName: 'Deutsche Bank',
    contactName: 'Hans Mueller',
    contactEmail: 'h.mueller@gamma.de',
    contactPhone: '+49 30 12345678',
    address: 'Berliner Str. 100, 10115 Berlin, Germany',
    taxId: 'DE123456789',
    paymentTerms: 'Net 30',
    linkedDocIds: ['fi-003'],
    linkedCaseIds: ['case-003'],
    linkedOpenItemIds: ['oi-005']
  },
  {
    id: 'V-10004',
    type: 'vendor',
    code: 'V-10004',
    name: 'Vendor Delta Corp',
    tenantId: BigInt(1001),
    companyCode: '1000',
    riskScore: 45,
    riskTrend: 'down',
    concentrationRisk: 'low',
    lastUpdated: '2026-01-30T08:20:00Z',
    openItemsTotal: 34200,
    openItemsCount: 1,
    overdueTotal: 0,
    overdueCount: 0,
    recentAnomaliesCount: 1,
    currency: 'USD',
    bankAccount: '****6789',
    bankName: 'Wells Fargo',
    contactName: 'Jennifer Smith',
    contactEmail: 'j.smith@deltacorp.com',
    contactPhone: '+1 555-0104',
    address: '789 Industrial Blvd, Chicago, IL 60601',
    taxId: '45-6789012',
    paymentTerms: 'Net 60',
    linkedDocIds: ['fi-004'],
    linkedCaseIds: ['case-004'],
    linkedOpenItemIds: ['oi-006']
  },
  {
    id: 'V-10005',
    type: 'vendor',
    code: 'V-10005',
    name: 'Supplier Epsilon',
    tenantId: BigInt(1001),
    companyCode: '3000',
    riskScore: 25,
    riskTrend: 'stable',
    concentrationRisk: 'low',
    lastUpdated: '2026-01-27T16:00:00Z',
    openItemsTotal: 0,
    openItemsCount: 0,
    overdueTotal: 0,
    overdueCount: 0,
    recentAnomaliesCount: 0,
    currency: 'USD',
    bankAccount: '****0123',
    bankName: 'Citibank',
    contactName: 'Michael Brown',
    contactEmail: 'm.brown@epsilon.com',
    contactPhone: '+1 555-0105',
    address: '321 Supply Chain Rd, Houston, TX 77001',
    taxId: '67-8901234',
    paymentTerms: 'Net 30',
    linkedDocIds: ['fi-005'],
    linkedCaseIds: ['case-005'],
    linkedOpenItemIds: []
  },
  {
    id: 'V-20001',
    type: 'vendor',
    code: 'V-20001',
    name: 'Global Vendor A',
    tenantId: BigInt(1002),
    companyCode: '4000',
    riskScore: 92,
    riskTrend: 'up',
    concentrationRisk: 'high',
    lastUpdated: '2026-01-30T10:30:00Z',
    openItemsTotal: 275000,
    openItemsCount: 1,
    overdueTotal: 0,
    overdueCount: 0,
    recentAnomaliesCount: 1,
    currency: 'USD',
    bankAccount: '****4321',
    bankName: 'HSBC',
    contactName: 'Sarah Johnson',
    contactEmail: 's.johnson@globalvendor.com',
    contactPhone: '+1 555-0201',
    address: '500 Global Plaza, Miami, FL 33101',
    taxId: '89-0123456',
    paymentTerms: 'Net 45',
    linkedDocIds: ['fi-006'],
    linkedCaseIds: ['case-006'],
    linkedOpenItemIds: ['oi-007']
  },
  {
    id: 'C-50001',
    type: 'customer',
    code: 'C-50001',
    name: 'Customer XYZ Corp',
    tenantId: BigInt(1001),
    companyCode: '1000',
    riskScore: 35,
    riskTrend: 'stable',
    concentrationRisk: 'medium',
    lastUpdated: '2026-01-26T11:30:00Z',
    openItemsTotal: 78500,
    openItemsCount: 2,
    overdueTotal: 25000,
    overdueCount: 1,
    recentAnomaliesCount: 0,
    currency: 'USD',
    bankAccount: '****9876',
    bankName: 'PNC Bank',
    contactName: 'David Lee',
    contactEmail: 'd.lee@xyzcorp.com',
    contactPhone: '+1 555-0301',
    address: '999 Customer Way, Seattle, WA 98101',
    taxId: '11-2233445',
    paymentTerms: 'Net 30',
    linkedDocIds: ['fi-011'],
    linkedCaseIds: [],
    linkedOpenItemIds: ['oi-008', 'oi-009']
  },
  {
    id: 'V-30001',
    type: 'vendor',
    code: 'V-30001',
    name: 'EU Supplier GmbH',
    tenantId: BigInt(1001),
    companyCode: '2000',
    riskScore: 55,
    riskTrend: 'stable',
    concentrationRisk: 'medium',
    lastUpdated: '2026-01-28T08:15:00Z',
    openItemsTotal: 125000,
    openItemsCount: 1,
    overdueTotal: 0,
    overdueCount: 0,
    recentAnomaliesCount: 0,
    currency: 'EUR',
    bankAccount: '****5678',
    bankName: 'Commerzbank',
    contactName: 'Anna Schmidt',
    contactEmail: 'a.schmidt@eusupplier.de',
    contactPhone: '+49 89 98765432',
    address: 'Maximilianstr. 50, 80538 Munich, Germany',
    taxId: 'DE987654321',
    paymentTerms: 'Net 30',
    linkedDocIds: ['fi-012'],
    linkedCaseIds: [],
    linkedOpenItemIds: ['oi-010']
  },
]

// Mock Entity Change Logs
export const mockEntityChangeLogs: EntityChangeLog[] = [
  {
    id: 'ecl-001',
    entityId: 'V-10001',
    timestamp: '2026-01-28T10:00:00Z',
    fieldName: 'bankAccount',
    beforeValue: '****1234',
    afterValue: '****4567',
    actor: 'System Import',
    actorType: 'system',
    source: 'SAP MM',
    severity: 'critical'
  },
  {
    id: 'ecl-002',
    entityId: 'V-10001',
    timestamp: '2026-01-25T14:30:00Z',
    fieldName: 'paymentTerms',
    beforeValue: 'Net 45',
    afterValue: 'Net 30',
    actor: 'AP_MANAGER',
    actorType: 'user',
    source: 'Manual Update',
    severity: 'warn'
  },
  {
    id: 'ecl-003',
    entityId: 'V-10002',
    timestamp: '2026-01-29T08:00:00Z',
    fieldName: 'bankAccount',
    beforeValue: '****5678',
    afterValue: '****8901',
    actor: 'Vendor Portal',
    actorType: 'system',
    source: 'Vendor Self-Service',
    severity: 'critical'
  },
  {
    id: 'ecl-004',
    entityId: 'V-10002',
    timestamp: '2026-01-20T11:00:00Z',
    fieldName: 'address',
    beforeValue: '123 Old Street, LA, CA',
    afterValue: '456 Commerce St, Los Angeles, CA 90001',
    actor: 'AP_USER01',
    actorType: 'user',
    source: 'Manual Update',
    severity: 'info'
  },
  {
    id: 'ecl-005',
    entityId: 'V-10003',
    timestamp: '2026-01-15T09:00:00Z',
    fieldName: 'paymentBlock',
    beforeValue: 'false',
    afterValue: 'true',
    actor: 'AI Agent',
    actorType: 'agent',
    source: 'Risk Assessment',
    severity: 'warn'
  },
  {
    id: 'ecl-006',
    entityId: 'V-10003',
    timestamp: '2026-01-29T11:00:00Z',
    fieldName: 'paymentBlock',
    beforeValue: 'true',
    afterValue: 'false',
    actor: 'Sarah Johnson',
    actorType: 'user',
    source: 'Manual Override',
    severity: 'info'
  },
  {
    id: 'ecl-007',
    entityId: 'V-20001',
    timestamp: '2026-01-30T09:00:00Z',
    fieldName: 'contactEmail',
    beforeValue: 'old.contact@globalvendor.com',
    afterValue: 's.johnson@globalvendor.com',
    actor: 'System Import',
    actorType: 'system',
    source: 'SAP MM',
    severity: 'info'
  },
  {
    id: 'ecl-008',
    entityId: 'C-50001',
    timestamp: '2026-01-26T10:00:00Z',
    fieldName: 'creditLimit',
    beforeValue: '100000',
    afterValue: '150000',
    actor: 'Credit Manager',
    actorType: 'user',
    source: 'Credit Review',
    severity: 'info'
  },
]

// Mock Open Items
export const mockOpenItems: OpenItem[] = [
  {
    id: 'oi-001',
    entityId: 'V-10001',
    entityName: 'Vendor Alpha Inc',
    docId: 'fi-001',
    docNumber: '1900001234',
    type: 'AP',
    amount: 125000,
    currency: 'USD',
    dueDate: '2026-02-27',
    daysPastDue: 0,
    status: 'open',
    disputeFlag: false,
    paymentBlock: true,
    blockReason: 'Pending duplicate invoice review',
    recommendedAction: 'request_approval',
    guardrailStatus: 'approval_required',
    companyCode: '1000',
    tenantId: BigInt(1001),
    clearingHistory: []
  },
  {
    id: 'oi-002',
    entityId: 'V-10001',
    entityName: 'Vendor Alpha Inc',
    docId: 'fi-007',
    docNumber: '1900001100',
    type: 'AP',
    amount: 45000,
    currency: 'USD',
    dueDate: '2026-01-15',
    daysPastDue: 16,
    status: 'partially_cleared',
    disputeFlag: false,
    paymentBlock: false,
    recommendedAction: 'auto_clear',
    guardrailStatus: 'allowed',
    companyCode: '1000',
    tenantId: BigInt(1001),
    clearingHistory: [
      { date: '2026-01-20', amount: 20000, clearingDoc: '1900001101' }
    ]
  },
  {
    id: 'oi-003',
    entityId: 'V-10001',
    entityName: 'Vendor Alpha Inc',
    docId: 'fi-009',
    docNumber: '1900001102',
    type: 'AP',
    amount: 45000,
    currency: 'USD',
    dueDate: '2026-02-21',
    daysPastDue: 0,
    status: 'open',
    disputeFlag: false,
    paymentBlock: false,
    recommendedAction: undefined,
    guardrailStatus: 'allowed',
    companyCode: '1000',
    tenantId: BigInt(1001),
    clearingHistory: []
  },
  {
    id: 'oi-004',
    entityId: 'V-10002',
    entityName: 'Supplier Beta LLC',
    docId: 'fi-002',
    docNumber: '1900001235',
    type: 'AP',
    amount: 89500,
    currency: 'USD',
    dueDate: '2026-03-15',
    daysPastDue: 0,
    status: 'open',
    disputeFlag: false,
    paymentBlock: false,
    recommendedAction: undefined,
    guardrailStatus: 'allowed',
    companyCode: '1000',
    tenantId: BigInt(1001),
    clearingHistory: []
  },
  {
    id: 'oi-005',
    entityId: 'V-10003',
    entityName: 'Contractor Gamma',
    docId: 'fi-003',
    docNumber: '1900001236',
    type: 'AP',
    amount: 52000,
    currency: 'EUR',
    dueDate: '2026-01-28',
    daysPastDue: 3,
    status: 'open',
    disputeFlag: true,
    paymentBlock: true,
    blockReason: 'Authorization limit exceeded',
    recommendedAction: 'request_approval',
    guardrailStatus: 'blocked',
    companyCode: '2000',
    tenantId: BigInt(1001),
    clearingHistory: []
  },
  {
    id: 'oi-006',
    entityId: 'V-10004',
    entityName: 'Vendor Delta Corp',
    docId: 'fi-004',
    docNumber: '1900001237',
    type: 'AP',
    amount: 34200,
    currency: 'USD',
    dueDate: '2026-03-30',
    daysPastDue: 0,
    status: 'open',
    disputeFlag: false,
    paymentBlock: false,
    recommendedAction: undefined,
    guardrailStatus: 'allowed',
    companyCode: '1000',
    tenantId: BigInt(1001),
    clearingHistory: []
  },
  {
    id: 'oi-007',
    entityId: 'V-20001',
    entityName: 'Global Vendor A',
    docId: 'fi-006',
    docNumber: '1900001239',
    type: 'AP',
    amount: 275000,
    currency: 'USD',
    dueDate: '2026-03-15',
    daysPastDue: 0,
    status: 'open',
    disputeFlag: false,
    paymentBlock: true,
    blockReason: 'Amount variance review',
    recommendedAction: 'create_case',
    guardrailStatus: 'approval_required',
    companyCode: '4000',
    tenantId: BigInt(1002),
    clearingHistory: []
  },
  {
    id: 'oi-008',
    entityId: 'C-50001',
    entityName: 'Customer XYZ Corp',
    docId: 'fi-011',
    docNumber: '1900001241',
    type: 'AR',
    amount: 53500,
    currency: 'USD',
    dueDate: '2026-02-25',
    daysPastDue: 0,
    status: 'open',
    disputeFlag: false,
    paymentBlock: false,
    recommendedAction: undefined,
    guardrailStatus: 'allowed',
    companyCode: '1000',
    tenantId: BigInt(1001),
    clearingHistory: []
  },
  {
    id: 'oi-009',
    entityId: 'C-50001',
    entityName: 'Customer XYZ Corp',
    docId: 'fi-011',
    docNumber: '1900001241-2',
    type: 'AR',
    amount: 25000,
    currency: 'USD',
    dueDate: '2026-01-20',
    daysPastDue: 11,
    status: 'open',
    disputeFlag: true,
    paymentBlock: false,
    recommendedAction: 'send_reminder',
    guardrailStatus: 'allowed',
    companyCode: '1000',
    tenantId: BigInt(1001),
    clearingHistory: []
  },
  {
    id: 'oi-010',
    entityId: 'V-30001',
    entityName: 'EU Supplier GmbH',
    docId: 'fi-012',
    docNumber: '1900001242',
    type: 'AP',
    amount: 125000,
    currency: 'EUR',
    dueDate: '2026-02-27',
    daysPastDue: 0,
    status: 'open',
    disputeFlag: false,
    paymentBlock: false,
    recommendedAction: undefined,
    guardrailStatus: 'allowed',
    companyCode: '2000',
    tenantId: BigInt(1001),
    clearingHistory: []
  },
  // Additional items for aging distribution
  {
    id: 'oi-011',
    entityId: 'V-10001',
    entityName: 'Vendor Alpha Inc',
    docId: 'fi-001',
    docNumber: '1900000900',
    type: 'AP',
    amount: 15000,
    currency: 'USD',
    dueDate: '2025-12-15',
    daysPastDue: 47,
    status: 'open',
    disputeFlag: false,
    paymentBlock: false,
    recommendedAction: 'escalate',
    guardrailStatus: 'approval_required',
    companyCode: '1000',
    tenantId: BigInt(1001),
    clearingHistory: []
  },
  {
    id: 'oi-012',
    entityId: 'V-10002',
    entityName: 'Supplier Beta LLC',
    docId: 'fi-002',
    docNumber: '1900000850',
    type: 'AP',
    amount: 8500,
    currency: 'USD',
    dueDate: '2025-11-01',
    daysPastDue: 91,
    status: 'open',
    disputeFlag: true,
    paymentBlock: true,
    blockReason: 'Under dispute',
    recommendedAction: 'create_case',
    guardrailStatus: 'blocked',
    companyCode: '1000',
    tenantId: BigInt(1001),
    clearingHistory: []
  },
  {
    id: 'oi-013',
    entityId: 'C-50001',
    entityName: 'Customer XYZ Corp',
    docId: 'fi-011',
    docNumber: '1900001000',
    type: 'AR',
    amount: 42000,
    currency: 'USD',
    dueDate: '2025-12-20',
    daysPastDue: 42,
    status: 'open',
    disputeFlag: false,
    paymentBlock: false,
    recommendedAction: 'send_reminder',
    guardrailStatus: 'allowed',
    companyCode: '1000',
    tenantId: BigInt(1001),
    clearingHistory: []
  },
]
