...
| Object Storage    | Local 개발 Provider, S3 Adapter     | 운영 전용 Quarantine Bucket, Compliance WORM 확장          |
| Malware/DLP       | Local 검사기, ClamAV Adapter        | 운영 AV/DLP/CDR 공급자와 비동기 Scan Worker                |
...
현재 R1은 Browser가 Messaging API에 Binary를 전송하고 API가 격리 저장 후 검사한다. JVM의
`byte[]` 처리 경계가 무제한 메모리 사용으로 이어지지 않도록 다음 Guard를 적용한다.

- `Content-Length` 필수, Tenant 정책과 `maximum-transfer-mb` 중 작은 값 적용
- Upload·Download 합산 `maximum-concurrent-transfers` Semaphore
- 초과 요청 `413`, 전송 Slot 포화 `429`와 `Retry-After`, Chunked 무길이 요청 `411`
- 기본값은 파일당 100MB, 동시 전송 2개이며 Pod Heap과 부하 시험 결과로 환경별 조정

100MB를 넘는 대용량 파일이 제품 요구사항으로 승인되면 API 메모리 상주 방식을 확장하지 않는다.
S3 Multipart Pre-signed Upload, Object Storage Event, 비동기 Scan Worker, 상태 Polling으로 전환하고
Quarantine Bucket Policy가 Browser Download를 거부하는지 침투 시험으로 확인한다.

현재 수명주기 기본값은 Upload 세션 10분, Download Grant 1분, 정리 주기 5분이다. 정리 Worker는
미연결 `QUARANTINED/CLEAN/REJECTED` Object를 `EXPIRED`로 선점하고 삭제하며, 만료되거나 오래전에
소비된 Grant도 제거한다. Object 삭제 실패는 Row를 남겨 다음 주기에 재시도한다.
