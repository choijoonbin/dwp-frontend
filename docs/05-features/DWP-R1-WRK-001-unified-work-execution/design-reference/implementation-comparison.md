> **HISTORICAL / SUPERSEDED · 2026-09-08 정정:** 아래 `2026-09-04 비교 기록`은 당시 비교와 폐기된 기능 판정을 보존합니다. 여기에 연결된 과거 캡처와 해시는 현재 UI를 증명하지 않습니다. 현재 계약은 먼저 적은 정정 내용을 따릅니다.

# Stitch 시안과 Work Hub 구현 비교 — 과거 기록

## 현재 제품 계약 · 2026-09-08

- Work는 통합업무함, 내 조치 대기, 오늘 계획, 진행 중, 응답 대기, 완료된 업무의 6개 메뉴·경로를 제공한다.
- 결재·서비스 원천 상세는 풍부한 읽기 전용 화면이며 승인·반려·서비스 응답은 검증된 원본 URL로 소유 앱에 handoff한다.
- 선택 업무 AI는 답변 읽기·출처 확인·복사·대화 이어가기를 제공하며 Work 폼으로 초안을 직접 적용하지 않는다.
- Flow 업무 contribution은 독립 Work 화면이 아니라 공유 Home이 소유한다.

## 2026-09-04 비교 기록

기준일: 2026-09-04. 대상은 확보한 Stitch 18개 프레임과 당시 `/work/queue` 구현이다. 화면의 문구·fixture·버튼은 현재 기능 계약이나 운영 증거로 사용하지 않는다.

## 당시 전체 판정

당시 구현은 Stitch의 역할 중심 목록–상세 구조와 주요 01–12 여정을 제품 코드에 반영했다고 판정했다. DWP design token과 공통 component로 다시 구성했고, 데이터·권한·명령은 당시 소유 API 계약에 맞췄다. 당시에는 중복 업무 홈과 6개 영구 하위 메뉴를 제거했다고 기록했으나, 이 메뉴 판정은 폐기됐고 현재 제품은 6개 Work 메뉴·경로를 제공한다.
...
| 번호               | 2026-09-04 당시 제품 구현                                                                                                                  | Stitch와 일치하는 경험                                                      | 의도적으로 달라진 내용                                                                      | 당시 판정                           |
| ------------------ | ------------------------------------------------------------------------------------------------------------------------------------------ | --------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------- | ----------------------------------- |
...
| 02 결재 상세       | 결재 역할·근거·상태를 표시하고 당시 계약이 허용한 owner action 또는 원본으로 handoff                                                       | 근거 우선 위계와 당시 사용자 조치 강조                                      | 법적 효력·감사 로그·엔진 동기화 보장, 계약 없는 범용 직접 승인/반려 제거                    | 조건부 구현                         |
...
4. `AVAILABLE`은 당시 접근 가능성이고 모든 mutation 권한이 아니다.
...
Meeting owner-source receiver는 내부 resolve/read만 구현됐다. CREATE는 `AUTHORITY_UNVERIFIED`, REASSIGN은 `TARGET_ELIGIBILITY_UNVERIFIED`로 fail-closed이며 당시 **NO-GO**로 기록됐다. Stitch의 버튼이나 fixture로 Meeting 생성·재배정을 활성 기능으로 표시하지 않는다.
