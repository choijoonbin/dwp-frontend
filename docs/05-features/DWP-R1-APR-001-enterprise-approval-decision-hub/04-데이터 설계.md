...
현재 로컬 R1 기준선은 Flyway `V1`~`V4`와 다음 물리 Table을 사용한다. 아래의 상세
...
| Form        | `apr_forms`, `apr_form_versions`                             | 한영 Label과 동적 Field Schema 버전                |
| Request     | `apr_requests`, `apr_request_payloads`, `apr_request_events` | 초안·상신 상태, Payload Hash, Append-only Timeline |
| Runtime     | `apr_steps`, `apr_tasks`                                     | 순차 단계 활성화와 역할 후보 Task                  |
| Governance  | `apr_policy_rules`, `apr_delegations`                        | 정책과 기간·Workflow 범위 위임                     |
...
- Migration ID: `dwp-approval-server`의 `V1__create_approval_decision_hub.sql`~
  `V4__remove_runtime_reference_transactions.sql`
