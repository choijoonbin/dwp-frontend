...
현재 로컬 R1 기준선은 Flyway `V1`~`V14`와 다음 물리 Table을 사용한다. 아래의 상세
...
| Evidence    | `sys_audit_outbox`, `sys_domain_event_*`, `apr_step_up_replay_ledger`, `apr_high_risk_idempotency_ledger`, `apr_recovery_auditor_assignment_events` | 감사·Domain Event, 고위험 Command replay·receipt, 복구 assignment 증적 |
| Scope fence | `apr_management_scope_schema_fence`                                                                                                                 | 관리 자산 resource-set Writer 활성화의 forward-only schema fence       |
...
### V12~V14 고위험 Command·관리 Scope 결속

- `V12`는 step-up challenge·nonce의 단일 사용 원장과 고위험 Command idempotency receipt를
  추가한다. Request hash 불일치, challenge replay, 완료 응답 중복은 Domain mutation 전에
  차단하며 원장에는 전체 업무 Payload를 저장하지 않는다.
- `V13`은 통합 전달 복구 감사자 선정을 lease·bounded retry·cooldown으로 비동기화하고,
  exhausted epoch와 자동 probe를 append-only Event로 남긴다. 원 사건 행위자와 감사자는
  동일할 수 없다.
- `V14`는 Workflow·Form·Category·Policy·Signature Provider·Request·Integration Event를
  exact `management_resource_set_key`에 결속한다. Workflow 위임은 표시용 key가 아니라
  tenant-bound immutable `workflow_id`를 권위로 사용하며, 정책·복구 Event FK도 Tenant를
  함께 검증한다.
