...
> **2026-09-07 판정 정정:** 이 문서의 기존 `구조 합격`은 승인 원본 디자인 충실도를 입증하지 못했다. U06–U15 20개를 재감사한 결과 레이아웃·콘텐츠·모바일 기본 노출의 실질 누락을 확인했다. 아래 표는 과거 구현/원본 추적 이력으로만 유지하며, 현재 합격 판정으로 사용하지 않는다. 최신 원인·보정·잔여는 `09-design-correction-20-screens-2026-09-07.md` 및 각 담당 검토 문서에 기록한다. 구현 snapshot hash를 원본과의 일치 증거로 대체하지 않는다.
