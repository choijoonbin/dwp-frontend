...
| 데이터·Sync·보존 Interface | Foundation 완료 | V1-V7, 영속 Version, RLS·Runtime Role, 외부 보존은 Gate     |
| API·권한·Tenant 격리       | Foundation 완료 | Idempotency·Cursor Reset·Tenant/User Scope                  |
...
따라서 **Direct Recipient 기반 In-app Foundation은 구현 완료**다. 다음 순서는 Production 증거와
People Audience Contract, 외부 Channel 순서이며, 실제 연결 전에는 UI·API Capability를 열지 않는다.
