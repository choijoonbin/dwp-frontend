/**
 * Anomalies 필터용 날짜 유틸 — KST 기준
 * range + from/to (API: range, from, to)
 */

export type AnomaliesPeriod = '1h' | '6h' | '24h' | '7d' | '30d' | '90d';
...
export const getDateRangeFromPeriod = (period: AnomaliesPeriod): { from: string; to: string } => {
  ...
  switch (period) {
    ...
    case '30d':
      ...
      break;
    case '90d':
      fromKST = new Date(nowKST.getTime() - 90 * 24 * 60 * 60 * 1000);
      break;
    default:
      fromKST = new Date(nowKST.getTime() - 24 * 60 * 60 * 1000);
  }
