...
import {
  ...
  AuditEventDrawer,
  AuditExportDialog,
  AuditSavedSearchDialog,
} from './audit-explorer-overlays';
import {
  OutcomeChip,
  RiskScore,
  SeverityChip,
...
export function AuditExplorer() {
  ...
  return (
    ...
    >
      ...
      <AuditEventDrawer
        selected={selected}
        canInvestigate={hasPermission('ADMIN.AUDIT_INVESTIGATE', 'UPDATE')}
        actionLabel={auditActionLabel}
        onClose={() => setSelected(null)}
        onPreserve={() => setCaseLinkOpen(true)}
        onCopy={copy}
      />
      <AuditExportDialog
        open={exportOpen}
        window={window}
        resultCount={eventsQuery.data?.totalElements ?? 0}
        format={exportFormat}
      ...
      <AuditSavedSearchDialog
        ...
        shared={savedSearchShared}
        canShare={hasPermission('ADMIN.AUDIT_CONFIGURE', 'MANAGE')}
        busy={saveSearchMutation.isPending}
        onClose={() => setSaveSearchOpen(false)}
        onNameChange={setSavedSearchName}
        onSharedChange={setSavedSearchShared}
        onSubmit={() => saveSearchMutation.mutate()}
      />
      <AuditCaseLinkDialog
        open={caseLinkOpen}
        ...
        caseId={linkCaseId}
        note={linkNote}
        busy={linkEventMutation.isPending}
        onClose={() => setCaseLinkOpen(false)}
        onCaseIdChange={setLinkCaseId}
        onNoteChange={setLinkNote}
        onSubmit={() => linkEventMutation.mutate()}
      />
    </Box>
  );
}
