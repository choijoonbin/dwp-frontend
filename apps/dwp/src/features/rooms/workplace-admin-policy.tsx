...
function PolicySection({ icon: Icon, title, children }: { icon: LucideIcon; title: string; children: React.ReactNode }) {
  return (
    <Box sx={{ border: 1, borderColor: 'divider', bgcolor: 'background.paper', p: { xs: 1.5, md: 2.25 } }}>
      <Stack direction="row" gap={1} alignItems="center" sx={{ mb: 2 }}>
        <Box sx={{ width: 34, height: 34, display: 'grid', placeItems: 'center', bgcolor: 'var(--dwp-product-soft)', color: 'var(--dwp-product-accent)' }}><Icon size={18} /></Box>
  ...
  return (
    <PageCanvas>
      <RoomsPageHeading
        ...
        actions={
          <ActionButton intent="primary" startIcon={<Save size={17} />} disabled={!dirty || !valid} loading={mutation.isPending} loadingLabel={t('actions.saving')} onClick={() => mutation.mutate()}>
            {t('actions.save')}
          </ActionButton>
        }
      />
      {query.isLoading && <Skeleton variant="rectangular" height={560} />}
      {query.isError && <Alert severity="error" action={<ActionButton intent="quiet" onClick={() => query.refetch()}>{t('actions.retry')}</ActionButton>}>{t('workplace.admin.policy.loadError')}</Alert>}
      {form && (
        <Stack spacing={1.5}>
          {!valid && <Alert severity="warning">{t('workplace.admin.policy.validationError')}</Alert>}
          <PolicySection icon={CalendarRange} title={t('workplace.admin.policy.bookingWindowTitle')}>
            <Typography variant="body2" color="text.secondary" sx={{ mb: 1 }}>{t('workplace.admin.policy.bookingWindowHint')}</Typography>
            <ToggleButtonGroup
              exclusive
          ...
          <PolicySection icon={Armchair} title={t('workplace.admin.policy.fixedSeatTitle')}>
            <FormControlLabel control={<Switch checked={form.allowAssignedDeskLending} onChange={(_, value) => patch('allowAssignedDeskLending', value)} />} label={t('workplace.admin.policy.allowAssignedDeskLending')} />
            <Typography variant="body2" color="text.secondary">{t('workplace.admin.policy.allowAssignedDeskLendingHint')}</Typography>
          </PolicySection>
          ...
          <PolicySection icon={Clock3} title={t('workplace.admin.policy.hoursTitle')}>
            <Box sx={{ display: 'grid', gridTemplateColumns: { xs: '1fr', md: 'repeat(4, 1fr)' }, gap: 1.5 }}>
              ...
              />
              <FormField type="number" label={t('workplace.admin.policy.minimumMinutes')} value={form.minimumBookingMinutes} onChange={(event) => patch('minimumBookingMinutes', Number(event.target.value))} inputProps={{ min: 15, max: 1440, step: 15 }} />
              <FormField type="number" label={t('workplace.admin.policy.maximumMinutes')} value={form.maximumBookingMinutes} onChange={(event) => patch('maximumBookingMinutes', Number(event.target.value))} inputProps={{ min: 15, max: 10080, step: 15 }} />
            </Box>
          </PolicySection>

          <PolicySection icon={UserRoundCheck} title={t('workplace.admin.policy.arrivalTitle')}>
            <Stack gap={1.25}>
              <FormControlLabel control={<Switch checked={form.requireCheckIn} onChange={(_, value) => patch('requireCheckIn', value)} />} label={t('workplace.admin.policy.requireCheckIn')} />
              <Box sx={{ display: 'grid', gridTemplateColumns: { xs: '1fr', md: '1fr 1fr' }, gap: 1.5 }}>
                <FormField type="number" disabled={!form.requireCheckIn} label={t('workplace.admin.policy.checkInLead')} value={form.checkInLeadMinutes} onChange={(event) => patch('checkInLeadMinutes', Number(event.target.value))} inputProps={{ min: 0, max: 240 }} />
                <FormField type="number" disabled={!form.requireCheckIn} label={t('workplace.admin.policy.autoRelease')} value={form.autoReleaseMinutes} onChange={(event) => patch('autoReleaseMinutes', Number(event.target.value))} inputProps={{ min: 0, max: 240 }} />
              </Box>
            </Stack>
          </PolicySection>
          ...
          <PolicySection icon={ShieldCheck} title={t('workplace.admin.policy.privacyTitle')}>
            <Stack>
              <FormControlLabel control={<Switch checked={form.showColleagueNames} onChange={(_, value) => patch('showColleagueNames', value)} />} label={t('workplace.admin.policy.showNames')} />
            </Stack>
          </PolicySection>
        </Stack>
      )}
