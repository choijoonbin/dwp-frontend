...
function Metric({
  ...
  return (
    <Box
      sx={{ border: 1, borderColor: 'divider', bgcolor: 'background.paper', p: 2, minHeight: 118 }}
    >
  ...
  return (
    <PageCanvas>
      ...
      {query.isLoading ? (
        ...
        >
          {[0, 1, 2, 3].map((value) => (
            <Skeleton key={value} variant="rectangular" height={118} />
          ))}
        </Box>
      ) : overview ? (
        <>
          ...
          >
            <Metric
              icon={Building2}
              label={t('workplace.admin.overview.activeSites')}
              value={overview.activeSites}
            />
            <Metric
              icon={Layers3}
              label={t('workplace.admin.overview.configuredFloors')}
              value={overview.configuredFloors}
            />
            <Metric
              icon={Monitor}
              label={t('workplace.admin.overview.reservable')}
              value={overview.reservableResources}
            />
            <Metric
              icon={CalendarCheck2}
              label={t('workplace.admin.overview.weekBookings')}
              value={overview.bookingsThisWeek}
            />
          </Box>
          <Box
            sx={{
            ...
            <Box sx={{ border: 1, borderColor: 'divider', bgcolor: 'background.paper', p: 2.25 }}>
              <Stack direction="row" gap={1} alignItems="center">
                ...
                <Typography variant="h6" fontWeight={750}>
                  {t('workplace.admin.overview.utilization')}
                </Typography>
              </Stack>
              ...
              <Typography variant="body2" color="text.secondary" sx={{ mt: 1.5 }}>
                {t('workplace.admin.overview.utilizationHint')}
              </Typography>
            </Box>
            <Box sx={{ border: 1, borderColor: 'divider', bgcolor: 'background.paper', p: 2.25 }}>
              <Stack direction="row" gap={1} alignItems="center">
              ...
              <Stack gap={1.4} sx={{ mt: 2 }}>
                <Stack direction="row" justifyContent="space-between">
                  <Typography variant="body2" color="text.secondary">
                    {t('workplace.admin.policy.bookingWindow')}
                  </Typography>
                  <Typography variant="body2" fontWeight={750}>
                    {t('workplace.admin.policy.days', { count: overview.policy.bookingWindowDays })}
                  </Typography>
                </Stack>
                <Stack direction="row" justifyContent="space-between">
                  <Typography variant="body2" color="text.secondary">
                    {t('workplace.admin.policy.activeLimit')}
                  </Typography>
                  <Typography variant="body2" fontWeight={750}>
                    {overview.policy.maximumActiveBookings}
                  </Typography>
                </Stack>
                <Stack direction="row" justifyContent="space-between">
                  <Typography variant="body2" color="text.secondary">
                    {t('workplace.admin.policy.checkIn')}
                  </Typography>
                  <Typography variant="body2" fontWeight={750}>
                    {overview.policy.requireCheckIn
                      ? t('workplace.admin.policy.enabled')
                      : t('workplace.admin.policy.disabled')}
                  </Typography>
                </Stack>
                <Stack direction="row" justifyContent="space-between">
                  <Typography variant="body2" color="text.secondary">
                    {t('workplace.admin.overview.assignedSeats')}
                  </Typography>
                  <Typography variant="body2" fontWeight={750}>
                    {overview.assignedResources}
                  </Typography>
                </Stack>
                <Stack direction="row" justifyContent="space-between">
                  <Typography variant="body2" color="text.secondary">
                    {t('workplace.admin.overview.checkedInToday')}
                  </Typography>
                  <Typography variant="body2" fontWeight={750}>
