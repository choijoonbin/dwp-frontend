...
import { productScopeIdentitiesAreKnownAndUnique } from './product-manifest';
import {
...
import { useSingleProductSurfaceTaskTelemetry } from './use-single-product-surface-task-telemetry';
...
} from './product-surface-high-risk-command-runtime';
import {
...
import type { ApprovalHighRiskOperationBinding } from './product-surface-high-risk-command-runtime';
import type { ProductSurfaceOperationIdentity } from './product-surface-operation-coordinator';
...
export function buildApprovalHighRiskActionEvaluationRequest(
  ...
  target = productSurfaceHighRiskOperationBinding(operation).target
): ProductSurfaceEvaluationRequest {
  const operationBinding = productSurfaceHighRiskOperationBinding(operation);
  return {
    subject: {
      type: 'PRODUCT',
    ...
    routeContractKey: operationBinding.routeContractKey,
...
export function useApprovalHighRiskCommand<TResult>({
  ...
  const highRiskOperation = productSurfaceHighRiskOperationBinding(operation);
  const operationTarget = highRiskOperation.target;
  const taskKind = resolveProductSurfaceTaskKind({
    productKey: operationTarget.productKey,
    surfaceKey: operationTarget.surfaceKey,
    routeContractKey: highRiskOperation.routeContractKey,
  });
  const {
