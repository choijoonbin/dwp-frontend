...
test('Source10 UI fixture: 원본·약관 고정, 동의 기본false와 focus복원, 단건 생성/동의/취소 및 부분 감사', async ({
  ...
  await expect(dialog).toHaveCount(0);
  await expect(panel.getByRole('button', { name: '본인 인증 후 서명', exact: true })).toBeEnabled();
...
test('Source10 UI fixture: 403/503 원본은 failclosed, 320px/200%/visible tooltip Axe와 최신200만 복구', async ({
  ...
  await expect
    .poll(() =>
      page.getByRole('tooltip').evaluate((element) => Number(getComputedStyle(element).opacity))
    )
    .toBe(1);
...
test('Source10 UI fixture: UNKNOWN 명시적retry만 원래동일key/body를 사용한다', async ({ page }) => {
  const { state, panel } = await setup(page);
  state.response = 'UNKNOWN';
  await panel.getByRole('button', { name: '서명 확인 시작', exact: true }).click();
  await expect(panel.getByRole('button', { name: '원래 명령 재시도', exact: true })).toBeVisible();
  expect(state.writes).toHaveLength(1);
